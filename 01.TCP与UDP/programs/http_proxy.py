...
def proxy_process( sock1, sock2 ):
    """在两个sockek之间转发数据：任何一个收到的，转发到另一个"""
    sel = DefaultSelector()
...
def http_proxy(sock_in, addr):
    ...
    try:
        ...
        sock_in.send( STATUS_502 )
        sock_in.close()
        return
    sock_remote.settimeout(0)

    if not is_http:
        sock_in.send( STATUS_200 )

    proxy_process( sock_in, sock_remote )
...
def start_server():
    ...
    while True:
        sock, addr = s.accept()
        t = threading.Thread(target=http_proxy, args=(sock, addr))
        t.start()
